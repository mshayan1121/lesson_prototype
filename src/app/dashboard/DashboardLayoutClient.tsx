...
const SIDEBAR_PREFERENCE_EVENT = 'sidebar-preference-change'
...
function persistCollapsedPreference(collapsed: boolean) {
  ...
  document.cookie = `${SIDEBAR_PREFERENCE_KEY}=${collapsed}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}; samesite=lax`
  window.dispatchEvent(new Event(SIDEBAR_PREFERENCE_EVENT))
}

function subscribeToSidebarPreference(onStoreChange: () => void) {
  function handleStorage(event: StorageEvent) {
    if (event.key === null || event.key === SIDEBAR_PREFERENCE_KEY) {
      onStoreChange()
    }
  }
...
function getClientCollapsedSnapshot(initialCollapsed: boolean) {
  const saved = localStorage.getItem(SIDEBAR_PREFERENCE_KEY)
  return saved === null ? initialCollapsed : saved === 'true'
}
...
export default function DashboardLayoutClient({
  ...
  const collapsed = useSyncExternalStore(
    ...
    () => getClientCollapsedSnapshot(initialCollapsed),
    () => initialCollapsed
  )
  ...
  return (
    ...
    >
      ...
      <aside className={cn(
        ...
        transitionsEnabled ? 'transition-[width] duration-[220ms] ease-[cubic-bezier(0.4,0,0.2,1)]' : ''
      )}>
      ...
      <div className={cn(
        ...
        ready ? 'visible' : 'invisible',
        transitionsEnabled ? 'transition-[margin] duration-[220ms] ease-[cubic-bezier(0.4,0,0.2,1)]' : ''
      )}>
        <main className="p-6">
          {children}
        </main>
